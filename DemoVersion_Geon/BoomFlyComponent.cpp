...
namespace TestGeon
{
	...
	void BoomFlyComponent::createExplosion()
	{
		...
		for (unsigned int i = 0; i < 3; ++i)
		{
			...
			{
				GameObject* explosion = ObjectManager::GetInstance()->CreateObject(SoulBeaterProcessor::mTempId++, 2.f);
				explosion->CreateComponent<Transform>(SoulBeaterProcessor::mTempId++);
				GridComponent* grid = explosion->CreateComponent<GridComponent>(SoulBeaterProcessor::mTempId++);
				SpriteRenderer* renderer = explosion->CreateComponent<SpriteRenderer>(SoulBeaterProcessor::mTempId++);
				CircleCollider* circleCollider = explosion->CreateComponent<CircleCollider>(SoulBeaterProcessor::mTempId++);
